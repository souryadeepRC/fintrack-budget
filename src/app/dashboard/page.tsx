...
import { Bot,Lightbulb, Plus, Sparkles, TrendingUp } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const currentMonthEnd = useMemo(() => endOfMonth(currentMonthStart), [currentMonthStart]);

  // Fetch current month expenses with strict query keys
  const { data: currentMonthExpenses = [], isLoading } = useQuery({
    queryKey: [
  ...
  const maxCategoryAmount = categoryData.length > 0 ? Math.max(...categoryData.map(d => d.amount)) : 0;

  // Handler for opening the Add Expense modal
  const handleOpenAddExpense = useCallback(() => {
    // TODO: Dispatch your uiSlice action here to open the modal
    // Example: dispatch(openModal('addExpense'));
        ...
        <div className='grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8'>
          ...
          <section className='col-span-1 lg:col-span-2 space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-150'>
            <div className='flex items-center gap-2 px-1'>
              ...
              <h2 className='text-xl font-bold text-slate-800'>Monthly Analytics</h2>
            </div>
            ...
            <div className='bg-white/80 backdrop-blur-md rounded-2xl border border-slate-200/60 p-6 shadow-sm'>
              <div className='mb-6'>
                <p className='text-sm font-medium text-slate-500 uppercase tracking-wider'>{format(currentMonthStart, 'MMMM yyyy')} Breakdown</p>
                <p className='text-3xl font-bold text-slate-900 mt-1'>₹{totalCurrentMonth.toLocaleString('en-IN')}</p>
              </div>
              
              {categoryData.length > 0 ? (
                <div className='space-y-4'>
                  {categoryData.map((category) => (
                    <div key={category.name} className='flex flex-col gap-1.5 group'>
                      <div className='flex justify-between items-center text-sm'>
                        <span className='font-medium text-slate-700 transition-colors group-hover:text-emerald-700'>{category.name}</span>
                        <span className='font-semibold text-slate-900'>₹{category.amount.toLocaleString('en-IN')}</span>
                      </div>
                ...
                <div className='py-10 text-center text-sm text-slate-500'>No expenses recorded this month yet.</div>
              )}
            </div>
          </section>
            ...
            <div className='bg-white/80 backdrop-blur-md rounded-2xl border border-slate-200/60 p-6 shadow-sm flex flex-col gap-4 h-[calc(100%-2.5rem)]'>
              ...
              <div className='p-4 bg-linear-to-br from-amber-50 to-orange-50 rounded-xl border border-amber-100/50 relative overflow-hidden'>
                ...
                <p className='text-sm text-amber-900 font-medium leading-relaxed relative z-10'>
                  "You've saved <span className='font-bold text-amber-600'>15% more</span> on Food & Dining compared to last month. Keep cooking at home to maintain this incredible streak!"
                </p>
              </div>
              ...
              <div className='p-4 bg-linear-to-br from-emerald-50 to-teal-50 rounded-xl border border-emerald-100/50 relative overflow-hidden flex-1'>
                ...
                <h3 className='text-xs font-bold text-emerald-700 uppercase tracking-wider mb-2'>Daily Motivation</h3>
                <p className='text-sm text-emerald-900 font-medium leading-relaxed relative z-10 italic'>
                  "A budget is telling your money where to go instead of wondering where it went. Every expense tracked is a step closer to financial freedom."
                </p>
              </div>
            </div>
          </section>
